...
def safe(score: float) -> float:
    try:
        ...
        return MIN_SCORE
    return max(MIN_SCORE, min(float(score), MAX_SCORE))
...
TASK_1 = {
    ...
    "instructions": (
        ...
        "and ISO numeric country code (e.g. 356 for India) from the profile. "
...
def grade_field(field_name: str, agent_value: Any, truth_value: Any, task_id: int) -> float:
    ...
    if field_name == "skills":
        ...
        raw = overlap / len(t_set)
        return safe(raw)

    if field_name == "phone":
        a_digits = "".join(filter(str.isdigit, a))
        t_digits = "".join(filter(str.isdigit, t))
        return MAX_SCORE if a_digits == t_digits else MIN_SCORE
    ...
    if field_name == "address":
        ...
        if not t_words:
            return MIN_SCORE
        raw = len(a_words & t_words) / len(t_words)
        return safe(raw)
...
def grade_submission(task_id: int, filled_fields: Dict[str, Any]) -> float:
    """Grade a filled form. Always returns strictly between 0.1 and 0.9."""
